...
import { useGetBadgeListMutation } from "./badgeApi";
...
export default function BadgeList() {
	...
	// project Api
	const [getBadgeList, { isLoading, isError, isSuccess, data: badgeData }] =
		useGetBadgeListMutation();

	useEffect(() => {
		getBadgeList({ page: 1, perPage: 10, filterText: "", project_id: id });
	}, []);

	if (isSuccess) {
		console.log(badgeData.list);
	}
			...
			<StyledTableContainer component={Paper}>
				<Table sx={{ minWidth: 500 }} aria-label="custom pagination table">
					...
					<TableBody>
						...
						{!isLoading && isSuccess && badgeData.list.length === 0 && (
							<TableRow>
								<TableCell colSpan={4} align="center">
									No Data Found
								</TableCell>
